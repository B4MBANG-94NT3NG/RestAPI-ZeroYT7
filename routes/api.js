...
var kuhong = db.get('ojan'); // jan diubah
	...
	console.log('WELCOME TO BTR API!') // boleh diubah
}
...
// Api Features :
        ...
        var randomTextNumber = random+randomlagi+'---------kuhong-api-storage'+'BTRAPI';
...
router.get('/flaming', async (req, res, next) => {
  ...
  try {
	 ...
	 var buffer = await (await fetch(`https://api.imgbb.com/1/upload?expiration=120&key=761ea2d5575581057a799d14e9c78e28&image=${json.result}&name=BTRAPI`)).json()
...
try {
  ...
  res.sendFile(__path + '/views/invalidKey.html')
}
})
//cecan
